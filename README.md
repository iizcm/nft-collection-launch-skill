...
| Field | Value |
| --- | --- |
| **Name** | `nft-collection-launch-skill` |
...
End-to-end launch NFT collection di EVM/RBH: gen asset (logo/banner/prereveal/10 char), build website (Vercel, distinct UX tiap project, bg lime #CCFF00), tulis item name+desc prereveal, dan 4 post shill (OpenSea link duluan). Pakai saat user mulai project NFT baru, minta buat website/logo/shill, atau ganti tema project.
...
cp -r nft-collection-launch ~/.hermes/skills/nft-collection-launch/

# Option B: Install directly from this repository's raw SKILL.md URL
<agent-cli> skills install https://raw.githubusercontent.com/iizcm/nft-collection-launch-skill/main/SKILL.md

# Option C: Clone this entire repo into your skills directory
git clone https://github.com/iizcm/nft-collection-launch-skill.git ~/.hermes/skills/nft-collection-launch/
```
...
```text
skill_view(name="nft-collection-launch")
```
...
```text
Use the nft collection launch skill to <describe your specific task here>
```
...
```text
Gunakan skill nft collection launch untuk <deskripsikan tugas spesifik Anda di sini>
```
